  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!profileRes.ok) {
          throw new Error(te('loadFailed', { page: 'data' }));
        }
        ...
        setError(err.message || te('loadFailed', { page: 'dashboard' }));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (error || !profile) {
    return (
      <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-4 rounded-xl max-w-xl mx-auto my-12 text-center">
        ...
        <p className="text-sm mb-4">{error || t('sessionExpired')}</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`flex items-center gap-1 px-3.5 py-1.5 rounded-full text-xs font-bold border ${
              ...
              {t('riskScoreLabel')}: {profile.riskScore}%
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex items-center justify-between hover:border-emerald-500/30 transition-all duration-300">
          <div className="space-y-1">
            ...
            <h3 className="text-3xl font-extrabold text-emerald-400">${safeFloat(profile.wallet.availableBalance).toFixed(2)}</h3>
            <p className="text-xs text-slate-500">{t('availableBalanceDesc')}</p>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex items-center justify-between hover:border-sky-500/30 transition-all duration-300">
          <div className="space-y-1">
            ...
            <h3 className="text-3xl font-extrabold text-sky-400">${safeFloat(profile.wallet.pendingBalance).toFixed(2)}</h3>
            <p className="text-xs text-slate-500">{t('pendingBalanceDesc')}</p>
          </div>
